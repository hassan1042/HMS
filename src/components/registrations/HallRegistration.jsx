...
const HallRegistration = () => {
  ...
  const [offers, setOffers] = useState([{ title: '', description: '', price: '', validity: '' }]);

  const available = true;

  const handleAddOffer = () => {
    setOffers([...offers, { title: '', description: '', price: '', validity: '' }]);
  };
      ...
      <form onSubmit={handleAddHall} className="hall-registration-form flex justify-center items-center space-x-4 flex-wrap">
        ...
        <div className="offers-section w-full">
          ...
          {offers.map((offer, index) => (
            <div key={index} className="offer-group flex flex-wrap space-x-4 mb-4">
              <input
                className={inputStyles}
                type="text"
              ...
              {/* <input
                ...
                onChange={(e) => handleOfferChange(index, 'validity', e.target.value)}
              /> */}
            </div>
          ))}
          <button
